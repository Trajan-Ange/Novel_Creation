...
- [ ] **重要度筛选**：Phase 2 提取时仅关注实质性新增/变化，忽略已有信息的重复描述，减少 Phase 3 无效更新
- [ ] **H5 — 对话生成上下文膨胀**：`settings.py` chat_generate 的 `get_all_settings` 对 10+ 角色项目可能超 30K tokens。按相关性筛选而非全量注入
- [ ] **H6 — 章节大纲生成上下文精简**：`outline.py` create_chapter 包含 3 个大纲 + 2 个章节全文 + 全部设定，可超 40K tokens。按相关性裁剪

### 健壮性

- [ ] **H7 — 时间线拆分逻辑加固**：支持 `##` 标题、无标题等更多 LLM 输出格式变体
- [ ] **SSE 自动重连**：客户端断连后指数退避重连
