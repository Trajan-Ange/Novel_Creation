...
// Original non-streaming generateOutline kept for backward compat
async function generateOutlineLegacy(level, vol, ch) {
  const project = AppState.currentProject;
  const detail = document.getElementById('outline-detail');

  if (level === 'book') {
    detail.innerHTML = '<div class="loading">正在生成全书大纲...</div>';
    const result = await API.outline.generateBook(project);
    if (result.success) {
      await loadOutlineTree(project);
      viewOutline('book');
    } else {
      detail.innerHTML = `<div class="error-message">生成失败：${result.error}</div>`;
    }
  } else if (level === 'volume') {
    const v = vol || parseInt(prompt('输入卷号：'));
    if (!v) return;
    detail.innerHTML = '<div class="loading">正在生成卷大纲...</div>';
    const result = await API.outline.generateVolume(project, v);
    if (result.success) {
      await loadOutlineTree(project);
      viewOutline('volume', v);
    } else {
      detail.innerHTML = `<div class="error-message">生成失败：${result.error}</div>`;
    }
  } else if (level === 'chapter') {
    const instruction = prompt('请输入特别要求（可选）：');
    detail.innerHTML = '<div class="loading">正在生成章节大纲...</div>';
    const result = await API.outline.generateChapter(project, vol, ch, instruction || '');
    if (result.success) {
      await loadOutlineTree(project);
      viewOutline('chapter', vol, ch);
    } else {
